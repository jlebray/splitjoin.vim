function! sj#crystal#SplitIfClause()
  let line    = getline('.')
...
function! sj#crystal#JoinIfClause()
  let line    = getline('.')
  let pattern = '\v^\s*(if|unless|while|until)'
...
function! sj#crystal#SplitTernaryClause()
  let line    = getline('.')
...
function! sj#crystal#JoinTernaryClause()
  let line    = getline('.')
...
function! sj#crystal#JoinCase()
  ...
  if line =~ '.*case'
    ...
    for body_line in lines
      ...
      if ! call('sj#crystal#JoinWhenThen', [])
...
function! sj#crystal#SplitCase()
  ...
  if line =~ '.*case'
    ...
    for body_line in lines
      call cursor(line_no + counter, 1)
      if call('sj#crystal#SplitWhenThen', [])
...
function! sj#crystal#SplitWhenThen()
  let line = getline('.')
...
function! sj#crystal#JoinWhenThen()
  let line = getline('.')
...
function! sj#crystal#SplitProcShorthand()
  let pattern = '(&:\k\+[!?]\=)'
...
function! sj#crystal#SplitBlock()
  let pattern = '\v\{\s*(\|.{-}\|)?\s*(.*)\s*\}'
  ...
  if sj#settings#Read('crystal_do_block_split')
...
function! sj#crystal#SplitCachingConstruct()
  let line = getline('.')
...
function! sj#crystal#JoinCachingConstruct()
...
function! sj#crystal#SplitOptions()
  " Variables:
  "
  ...
  call sj#PushCursor()
  let [from, to] = sj#argparser#crystal#LocateHash()
  call sj#PopCursor()

  if from < 0 || !sj#CursorBetween(from, to)
    call sj#PushCursor()
    let [from, to, function_type] = sj#argparser#crystal#LocateFunction()
    call sj#PopCursor()
  ...
  let [from, to, args, opts, hash_type] = sj#argparser#crystal#ParseArguments(from, to, getline('.'))

  if len(opts) < 1 && len(args) > 0 && option_type == 'option'
    " no options found, but there are arguments, split those
    let replacement = join(args, ",\n")

    if !sj#settings#Read('crystal_hanging_args')
      let replacement = "\n".replacement."\n"
    elseif len(args) == 1
  ...
  " add opening brace
  if sj#settings#Read('crystal_curly_braces')
  ...
  else " !sj#settings#Read('crystal_curly_braces')
  ...
  " add closing brace
  if !sj#settings#Read('crystal_curly_braces') && option_type == 'option' && function_type == 'with_round_braces'
    " no need to add anything
  elseif sj#settings#Read('crystal_curly_braces') || option_type == 'hash' || len(args) == 0
    if sj#settings#Read('crystal_trailing_comma') || sj#settings#Read('trailing_comma')
      let replacement .= ','
    endif
...
function! sj#crystal#SplitArray()
  let [from, to] = sj#LocateBracesAroundCursor('[', ']', [
        \ 'crystalInterpolationDelimiter',
        \ 'crystalString',
        \ 'crystalStringDelimiter',
        \ 'crystalSymbolDelimiter'
        \ ])
  ...
  let [from, to, args; _rest] = sj#argparser#crystal#ParseArguments(from + 1, to - 1, getline('.'))
  if from < 0
    return 0
  endif
...
function! sj#crystal#JoinArray()
  ...
  if index(['crystalStringDelimiter', 'crystalSymbolDelimiter'], syntax_group) >= 0
    return 0
  endif
...
function! sj#crystal#JoinContinuedMethodCall()
  if getline('.') !~ '\.$'
    return 0
  endif
...
function! sj#crystal#JoinHeredoc()
  let heredoc_pattern = '<<-\?\([^ \t,]\+\)'
...
function! sj#crystal#SplitString()
  ...
  if sj#settings#Read('crystal_heredoc_type') == '<<-'
    call sj#ReplaceCols(match_start, match_end - 1, '<<-EOF')
    let replacement = getline('.')."\n".string_body."EOF"
    call sj#ReplaceMotion('V', replacement)
  elseif sj#settings#Read('crystal_heredoc_type') == '<<'
    call sj#ReplaceCols(match_start, match_end - 1, '<<EOF')
    let replacement = getline('.')."\n".string_body."EOF"
    call sj#ReplaceMotion('V', replacement)
    exe (line('.') + 1).','.(line('.') + 2).'s/^\s*//'
  else
    throw 'Unknown value for option "crystal_heredoc_type", "'.g:splitjoin_crystal_heredoc_type.'"'
  endif

  return 1
endfunction

function! sj#crystal#SplitArrayLiteral()
  if synIDattr(synID(line('.'), col('.'), 1), "name") !~ 'crystal\(String\|Symbol\)\%(Delimiter\)\='
    return 0
  endif
...
function! sj#crystal#JoinArrayLiteral()
  let syntax_group = synIDattr(synID(line('.'), col('.'), 1), "name")
  if index(['crystalStringDelimiter', 'crystalSymbolDelimiter'], syntax_group) < 0
    return 0
  endif
...
function! s:FindComments(start_line_no, end_line_no)
  ...
  for lineno in range(a:start_line_no, a:end_line_no)
    ...
    while search('\s*#.*$', 'W', lineno) > 0
      ...
      normal! f#
      if synIDattr(synID(lineno, col('.'), 1), "name") == 'crystalComment'
